...
def perm(listVar):
    retList = []    #用于存储排列组合的结果
    if listVar == []: #判断传参是不是空列表
        return [listVar] #返回的[[]]

    for i in range(len(listVar)):  #让所有的数字做一次开头
        #去掉一个节点后，获取剩余节点的列表
        restList = listVar[:i] + listVar[i+1:]

        #perm(restList)--》通过递归，把剩余所有元素的全排列获取到
        #遍历一下和x元素做多次组合，形成所有元素的全排列
        for x in perm(restList): 
            #上一步去掉的节点作为第一个元素，排列剩余元素产生的全排列
            retList.append(x+listVar[i:i+1])
    return retList
